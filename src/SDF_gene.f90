...
IF ((flag.eq.1).or.(flag.eq.3)) THEN
   ...
   Num=NUmMol(1)
ELSE
   OPEN(40,FILE="Geometry_molecule2.pdb",STATUS='OLD', ACTION='READ')
   Num=NUmMol(2)
END IF
...
!el vector normal que define el plano de rotación será el formado por los átomos 1 y 2, siendo el átomo 1 el que está en el origen
vn(1)=Acopy(2,1)
vn(2)=Acopy(2,2)
vn(3)=Acopy(2,3)
...
!Las proyecciones sobre el plano serán los vectores menos las proyecciones sobre el vector normal
!Buscamos la proyeccion sobre el vector normal al plano
...
!por ultimo, volvemos a trasladar el átomo A1 a su posición original.

IF ((Ocopy(1)**2+Ocopy(2)**2+Ocopy(3)).lt.dab**2) THEN
Do l=1, Num
...
WRITE(100+flag,'(A31,3F7.3,A24)')"ATOM      7  F   MOL           ", Ocopy(1), Ocopy(2), Ocopy(3), "  1.00  0.00           F"
 SELECT CASE (flag)
 CASE (1)
  sdf1=sdf1+1
 CASE (2)
